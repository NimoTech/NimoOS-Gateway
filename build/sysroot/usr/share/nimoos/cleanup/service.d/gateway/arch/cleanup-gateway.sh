...
readonly CASA_EXEC=nimoos-gateway
readonly CASA_SERVICE=nimoos-gateway.service
...
if [[ -z "${CASA_CONF}" ]]; then
    CASA_CONF=/etc/nimoos/gateway.ini
fi
...
rm -rvf /var/run/nimoos/gateway.pid
rm -rvf /var/run/nimoos/management.url
rm -rvf /var/run/nimoos/routes.json
rm -rvf /var/run/nimoos/static.url
rm -rvf /var/lib/nimoos/www
